...
void accUpdate(timeUs_t currentTimeUs)
{
    ...
    for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
        acc.accADCf.v[axis] = rpmFilterApply(axis, acc.accADC.v[axis], RPM_FILTER_ACCEL);
        acc.accADCf.v[axis] = accelerationRuntime.notchFilterApplyFn((filter_t *)&accelerationRuntime.notchFilter[axis], acc.accADCf.v[axis]);

        DEBUG_SET(DEBUG_ACCELEROMETER, axis, acc.accADCf.v[axis]);
    }
#endif
}
